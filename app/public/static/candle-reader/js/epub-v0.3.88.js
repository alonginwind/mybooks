...
})(Object(typeof self !== 'undefined' ? self : this), (__WEBPACK_EXTERNAL_MODULE__838__) => {
  ...
  function handler() {
    if (this.readyState === XMLHttpRequest.DONE) {
      ...
      if (this.status === 200 || this.status === 0 || responseXML) {
        ...
        // Get the actual Content-Type from response header
        ...
        if (responseXML) {
          ...
          // If no type detected from extension, try to determine from Content-Type header
          if (contentType.indexOf("application/xhtml+xml") > -1 || contentType.indexOf("application/xml") > -1 || contentType.indexOf("text/xml") > -1) {
            r = parse(this.response, "application/xhtml+xml");
          } else if (contentType.indexOf("text/html") > -1) {
            r = parse(this.response, "text/html");
          } else {
            // Default: try to parse as XML if content looks like XML
            if (typeof this.response === "string" && (this.response.trim().indexOf("<?xml") === 0 || this.response.trim().indexOf("<") === 0)) {
              r = parse(this.response, "application/xhtml+xml");
            } else {
              r = this.response;
            }
...
class Archive {
  ...
  handleResponse(response, type) {
    ...
    if (type == "json") {
      ...
      // If no type detected from extension, try content sniffing
